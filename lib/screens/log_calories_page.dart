...
class _LogCaloriesPageState extends State<LogCaloriesPage> {
  ...
  void initState() {
    super.initState();
    _refreshAll();
    _searchTemplateCtrl.addListener(() => setState(() {
      _templates = _svc.searchTemplates(_searchTemplateCtrl.text);
    }));
    _searchComponentCtrl.addListener(() => setState(() {
      _components = _svc.searchComponents(_searchComponentCtrl.text);
    }));
  ...
  Future<void> _refreshAll() async {
    setState(() {
      _templates = _svc.getAllTemplates();
      _recentTemplates = _svc.recentTemplates();
      _components = _svc.getAllComponents();
  ...
  Future<void> _logCurrentMeal({String? templateId, required String name}) async {
    ...
    await _svc.logMealFromLines(
      templateId: t.id,
      name: t.name,
      lines: lines,
    );

    setState(() => _msg = 'Logged ${t.name} • ${_totalMass().toStringAsFixed(0)} g • ${_totalKcal().toStringAsFixed(0)} kcal');
    await _refreshAll();
  }
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(controller: kcalCtrl, decoration: const InputDecoration(labelText: 'kcal per 100g'), keyboardType: TextInputType.number),
          ],
        ),
        actions: [
  ...
  Future<void> _editTemplate(MealTemplate t) async {
    ...
    _lines
      ..clear()
      ..addAll(t.lines.map((line) {
        final comp = _svc.getComponent(line.componentId);
        return _LineEditor(component: comp, grams: line.grams);
      }));
    _mealNameCtrl.text = t.name;
    setState(() {});
  }
  ...
  Future<void> _editLog(MealLog log) async {
    ...
    for (final s in snapshot) {
      ...
      final comp = _svc.getAllComponents().firstWhere(
        ...
        orElse: () => FoodComponent(id: 'adhoc:${s.name}', name: s.name, kcalPer100g: s.kcalPer100g),
      );
              ...
              child: Column(
                children: [
                      ...
                      children: [
                        Expanded(
                          child: DropdownButton<FoodComponent>(
                            value: le.component is FoodComponent && !(le.component!.id.startsWith('adhoc:')) ? le.component : null,
                            hint: Text(le.component?.name ?? 'Component'),
                            items: _svc.getAllComponents().map((c) => DropdownMenuItem(value: c, child: Text(c.name))).toList(),
                        ...
                        IconButton(
                          ...
                          onPressed: () => setStateDialog(() => editors.removeAt(idx)),
                        ),
                      ],
                    );
                  }),
                  ...
                  Align(
                    ...
                    child: Text('Total: ${dialogKcal().toStringAsFixed(0)} kcal'),
                  ),
                  const SizedBox(height: 8),
                  OutlinedButton.icon(
                    icon: const Icon(Icons.add),
                    label: const Text('Add component'),
                    onPressed: () => setStateDialog(() => editors.add(_LineEditor())),
                  ),
                ],
              ),
            ),
        ...
        child: ListView(
          children: [
            SegmentedButton<LogMode>(
              segments: const [
                ButtonSegment(value: LogMode.newMeal, label: Text('Build meal')),
                ButtonSegment(value: LogMode.savedMeal, label: Text('Saved meals')),
              ],
              selected: {_mode},
            ...
            if (_mode == LogMode.newMeal) ...[
              ...
              Wrap(
                ...
                children: _svc.getAllComponents().take(8).map((c) => ChoiceChip(
                  label: Text(c.name),
                  selected: false,
              ...
              ..._lines.asMap().entries.map((entry) {
                final idx = entry.key; final le = entry.value;
                return Card(
                  child: Padding(
              ...
              const SizedBox(height: 12),
              Text('Total: ${totalMass.toStringAsFixed(0)} g • ${totalKcal.toStringAsFixed(0)} kcal',
            ...
            if (_mode == LogMode.savedMeal) ...[
              ...
              if (_recentTemplates.isNotEmpty) ...[
                ...
                Wrap(
                  spacing: 8, runSpacing: 8,
                  children: _recentTemplates.map((t) => ChoiceChip(
                    label: Text(t.name),
                    ...
                    onSelected: (_) => setState(() {
                      ...
                      _lines
                        ..clear()
                        ..addAll(t.lines.map((l) => _LineEditor(
                          component: _svc.getComponent(l.componentId),
                          grams: l.grams,
                        )));
                      _mealNameCtrl.text = t.name;
                    }),
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<MealTemplate>(
                      ...
                      decoration: const InputDecoration(labelText: 'Select saved meal'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  IconButton(
                    ...
                    onPressed: _selectedTemplate == null ? null : () => _editTemplate(_selectedTemplate!),
                  ),
                  IconButton(
                    tooltip: 'Delete saved meal',
                    icon: const Icon(Icons.delete_forever),
                    onPressed: _selectedTemplate == null ? null : () => _deleteTemplate(_selectedTemplate!),
                  ),
                ],
              ),
              const SizedBox(height: 12),
            ...
            if (_msg != null) ...[
              const SizedBox(height: 12),
              Text(_msg!, style: TextStyle(color: _msg!.startsWith('Error') ? Colors.red : Colors.green)),
            ],

            const Divider(height: 32),

            Text('Today\'s logged meals', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: 8),
            ..._todayLogs.map((log) {
              final breakdown = (log.snapshot ?? [])
                  .map((s) => '${s.name}: ${s.grams.toStringAsFixed(0)}g • ${s.kcal.toStringAsFixed(0)} kcal')
                  .join('  •  ');
              return Card(
                child: ListTile(
                  leading: const Icon(Icons.restaurant_menu),
                  title: Text(log.name),
                  subtitle: Text('${log.totalMassGrams?.toStringAsFixed(0) ?? log.massGrams.toStringAsFixed(0)} g • ${log.kcal.toStringAsFixed(0)} kcal\n$breakdown'),
                  isThreeLine: true,
                  trailing: Wrap(
                    spacing: 6,
                    children: [
                      IconButton(icon: const Icon(Icons.edit), tooltip: 'Edit logged meal', onPressed: () => _editLog(log)),
                      IconButton(icon: const Icon(Icons.delete), tooltip: 'Delete', onPressed: () => _deleteLog(log)),
                    ],
                  ),
                ),
              );
